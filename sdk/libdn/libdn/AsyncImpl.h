...
namespace libdn {
	...
	extern std::queue<AsyncCallback*> _asyncCallbacks;

	template <class T>
	class AsyncImpl : public Async<T>, public AsyncCallback {
	private:
		void(__cdecl* _callback)(Async<T>*);
		T* _result;
		void* _userData;
		bool _freeResult;
		bool _isReferencedByCB;

		void(__cdecl* _timeoutCallback)(Async<T>*);
	...
	public:
		...
		virtual void SetCallback(void(__cdecl* callback)(Async<T>*), void* userData) {
			_callback = callback;
			_userData = userData;
			_isReferencedByCB = true;
		}

		virtual void SetTimeoutCallback(void(__cdecl* callback)(Async<T>*), unsigned int timeout) {
			_timeout = timeout;
			_start = GetTickCount();
